...
form!: FormGroup;
  ...
  ngOnInit(): void {
    ...
    this.form = this.fb.group({
      naziv: [initial.nazivListe, [Validators.required, Validators.minLength(3)]]
    });
  ...
  openFilmSearch(): void {
    const ref = this.dialog.open(SearchFilmComponent, {
      width: '90vw', maxWidth: '1200px', height: '85vh', maxHeight: '85vh',
  ...
  removeFilm(f: Film): void {
    ...
    if (this.paginator && this.filmsDS.data.length <= this.paginator.pageIndex * this.paginator.pageSize) {
      this.paginator.previousPage();
    }
  }

  clearAll(): void { this.filmsDS.data = []; }

  save(): void {
    if (this.form.invalid || this.filmsDS.data.length === 0) {
      this.form.markAllAsTouched();
      this.snack.open('Popuni naziv i dodaj bar jedan film.', 'OK', { duration: 2500 });
      return;
    }
